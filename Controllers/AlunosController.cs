        ...
        try
        {
            using (var stream = new MemoryStream())
            {
                ...
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    {
                        var headerValue = NormalizarCabecalho(worksheet.Cells[1, col].Value?.ToString());
                    ...
                    for (int row = 2; row <= rowCount; row++)
                    {
                        try
                        {
                            ...
                            var endereco = worksheet.Cells[row, headers["endereco"]].Value?.ToString()?.Trim() ?? "";
                            var numeroEndereco = worksheet.Cells[row, headers["numero"]].Value?.ToString()?.Trim() ?? "";
                            var bairro = worksheet.Cells[row, headers["bairro"]].Value?.ToString()?.Trim() ?? "";
                            var municipio = worksheet.Cells[row, headers["municipio"]].Value?.ToString()?.Trim() ?? "";
                            ...
                            var serie = worksheet.Cells[row, headers["serie"]].Value?.ToString()?.Trim() ?? "";
                            ...
                            var numPessoasStr = worksheet.Cells[row, headers["numero de pessoas na casa"]].Value?.ToString()?.Trim();
